...
import { Search, Plus, Pin, Filter, Archive, Loader2, ArrowLeft } from "lucide-react";
...
const Messages = () => {
  ...
  return (
    <MainLayout 
      title={isMobile && currentConversation && !showConversationList ? currentConversation.name : "MESSAGES"}
      backButton={isMobile && currentConversation && !showConversationList}
      icons={!isMobile}
    >
      <div className="flex h-[calc(100vh-4rem)]">
        {(!isMobile || showConversationList) && (
          <div className={`${isMobile ? 'w-full' : 'w-96'} border-r border-white/10 overflow-y-auto flex flex-col`}>
              ...
              <div className="flex gap-2 overflow-x-auto pb-2">
                {tabs.map(tab => (
                  <Button 
                    ...
                    size="sm" 
                    className={`rounded-full flex items-center whitespace-nowrap ${activeTab === tab.id ? "" : "bg-white/5 text-gray-300 hover:bg-white/10"}`} 
                    onClick={() => setActiveTab(tab.id)}
                  >
                    {tab.icon ? tab.icon : tab.label}
                    {tab.count > 0 && (
                      <Badge 
                        variant={activeTab === tab.id ? "tier" : "notification"} 
                        className="ml-1 min-w-5 text-center"
                      >
                        {tab.count}
                      </Badge>
                    )}
